...
internal static class ProjectileGeometry
{
        ...
        float cameraZoom)
    {
        ...
        // Capped at the pawn's own apparent-scale ceiling, not just floored at
        // zero: a pawn does not scale by the raw camera zoom, it scales by
        // PawnGeometry.ResolveApparentScale, which saturates at a zoom of
        // 2.40. Below that saturation point the raw zoom is already the
        // smaller of the two, so this changes nothing there and a pulled-out
        // shot keeps shrinking toward the MinimumDimension floor exactly as
        // before. Above it, without this cap, the prop kept growing with the
        // raw zoom while every pawn on screen stopped growing, so a spear at
        // the camera's tightest zoom drew about three and a half times a
