# Projectile props and embedded projectiles smoke — closed 2026-08-13
...
All eight of this family's rows are lifted into this record as `PASS`, but they
were not all passed against the same build. Seven passed on the first sitting.
The eighth, `PP-3`, failed that sitting, was fixed, and passed a second sitting
later the same day. Read the "How `PP-3` closed" section below before treating
the eight passes as one event.
...
`PP-3` did not pass on the first sitting. The tester's finding was that the
in-flight projectile "looks too big even when zoomed in" — a spear reading as
longer than the warriors it flew past. That is the opposite of the failure the
row is written against, so the finding sat outside what the row itself covers,
and the row stayed live in the checklist rather than being lifted here.

The cause was that a pawn scales by a clamped, saturating function of the camera
zoom while an in-flight prop scaled by the raw zoom, so above a zoom of `2.40`
the pawns stopped growing and the prop did not. The fix caps the prop at the
pawns' own apparent-scale ceiling. It shipped on 2026-08-13 as commit `c772849`
under a green `./scripts/verify.ps1` whose seed-1 digests did not move, and the
reasoning is recorded in the archived plan "Projectile prop scale", named here
in prose rather than linked. A person then re-ran `PP-3` against that build and
passed it, checking both ends of the zoom range in one sitting.
...
- **No agent may later enrich the `Actual` cells above.** The tester gave one
  ...
  one.
- **These eight passes record two builds, not one.** The seven above were
  observed before the in-flight prop was capped; `PP-3` was observed after. The
  cap changes only how large an in-flight prop draws above a zoom of `2.40`, so
  it does not disturb what the other seven rows watched, but no row here is
  evidence about any later build. If the in-flight or embedded projectile
  rendering is retuned again, all eight rows say nothing about the new values.
