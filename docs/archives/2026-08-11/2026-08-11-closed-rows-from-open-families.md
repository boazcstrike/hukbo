...
| SD-2 | Watch a squad path across the 26.57-degree diagonal wall | The funnel path visibly follows the wall as a straight line rather than a staircase | This row was recorded `BLOCKED` on 2026-08-11, on the tester's report "i am unsure which is which" — investigation found that `SandataGame.DrawOrderPath` drew only the player's own right-click polyline, and that no autonomous group path was ever drawn anywhere in `Sandata.Client`. **Fixed the same day**: `SandataSimulation.GetPublishedPath` opened the read path and `SandataGame.DrawPublishedPaths` draws it as a dashed line under the pawns, so the autonomous route and a player's own drawn path differ by shape rather than only by colour. The tester re-checked the row on 2026-08-12, judging the dashed line rather than the solid one, and reported: "good". The earlier observation is kept here in full so the reversal is visible rather than silent. | PASS |
...
| SD-7a | View a friendly and a hostile contact side by side in `night-ops`, then judge them again ignoring colour | The two are distinguishable at a glance, and remain distinguishable by shape alone | This row was recorded `FAIL` on 2026-08-11, on the tester's report that the two were distinguishable at a glance but "not distinguishable by shape" with colour ignored — the row requires both halves, and the colour-independent half is the accessibility half, so the row failed. **Fixed the same day**: a friendly keeps its square ground ring and gains a head pip, a hostile gets the same ring turned forty-five degrees into a diamond. The tester re-checked the row on 2026-08-12 and reported it passing: "passed". The earlier `FAIL` observation is kept here in full so the reversal is visible rather than silent, exactly as `SD-1`'s entry above. | PASS |
| SD-8 | Click an operator that is holding position | The inspector explains the hold: reason code, path state, and weapon chain phase | This row was recorded `BLOCKED` on 2026-08-11, for two independent reasons that neither masked the other: a click selected nothing because a click's marquee is zero pixels in both dimensions and `Rectangle.Contains` is false for every point inside it, and the inspector had no font to draw with. **Both fixed the same day.** The tester re-checked the row on 2026-08-12 and reported it passing: "passed". Four of the eleven inspector rows — the slot index and the three order-layer rows — still render their documented empty form, because no order layer exists yet to source them; that is expected and is not a regression. | PASS |
