...
| [ ] | T12 | Append `ClashBladeHard = 9`, `ClashBladeSoft = 10`, `ClashShield = 11` to `GameSoundId` **and** add all three to `SoundCatalog.AllSounds` with base names `clash-blade-hard`, `clash-blade-soft`, `clash-shield`, leaving `IsHitLocationDriven` false. The two must land together: `SoundCatalogTests.AllSounds_ListsEveryDeclaredSlotExactlyOnce` enumerates the enum, so appending members alone fails B0. They move to Phase 0 because the Phase 3b test task references them, and revision 3 had that task depended on by the very task that added them. | modify `src/Hukbo.Client/Audio/AudioTypes.cs`; modify `src/Hukbo.Client/Audio/SoundCatalog.cs`; modify `src/Hukbo.Client/UI/SoundLogPanel.Layout.cs`; modify `tests/Hukbo.Client.Tests/SoundCatalogTests.cs` | — | CLIENT-AUDIO | GUARD `AllSounds_ListsEveryDeclaredSlotExactlyOnce`, `AllSounds_ContainsTheThreeClashSlots`, `GetBaseName_NamesEveryClashSlot`, and `GetVariantPrefix` still throws for a clash slot. **Also add three `false` rows to `IsHitLocationDriven_IsTrueOnlyForTheFourWeaponSlots` at `SoundCatalogTests.cs:109-121`**: it is a nine-row `[Theory]` rather than an enumeration, so it silently would not cover the three new slots. No other *consumer* needs touching — every `AllSounds` consumer in production and in tests is written against `AllSounds.Count` and none hard-codes nine, which is exactly why the enum and the catalog entries land together. **But the panel geometry does.** `SoundLogPanel.Layout.cs` caps the expected-files section at `available - Gap - reservedCueHeight`, which is 155 pixels at the default `420x288`. A twelve-slot catalog wants `SectionHeaderHeight + 12 * BindingRowHeight`; at the old row height of 15 that is 196, so the list silently truncated to nine rows and hid the three clash slots in the one panel whose purpose is documenting what to name a file. Pre-existing `SoundLogPanelTests.CalculateLayout_ShowsEveryExpectedFileNameAtTheDefaultSize` at `SoundLogPanelTests.cs:326` exists to catch exactly that and went red. **Resolution: `BindingRowHeight` 15 to 11**, giving 148 and twelve visible rows without taking a row from the cue log's reserved minimum. Chosen over a taller panel (which pushes on `RightColumnSplit` and its neighbours) and over a scrollable bindings list (largest change, and a slot you must scroll to is weaker documentation than one you can see). Whether 11-pixel rows read cleanly is visual and belongs in the smoke checklist, not in a test. **DEFERRED by owner decision, 2026-07-27. Nothing in this task ships.** The three clash slots are not added to `GameSoundId` or `SoundCatalog`, no clash cue is mapped, and no WAV is authored. A clash resolves in the simulation and names itself in the battle event log; the audio layer stays silent. Two reasons. First, the owner wants each sound decided individually against a real battle rather than authored up front. Second, the panel has no room: after `main`'s font overhaul the expected-files section caps at 200 pixels at the real `420x396` and the nine current slots need exactly 200 — zero slack — with row height now derived from measured Caption-rung line spacing, so it cannot shrink without clipping descenders. Any tenth slot silently hides itself, which is the one failure `SoundLogPanelTests.CalculateLayout_ShowsEveryExpectedFileNameAtTheDefaultSize` exists to prevent. Solving that needs a taller panel (takes height from the event log, touches `RightColumnSplit`), a scrollable list, or dropping the cue log's reserved rows — a decision worth making once for however many slots are eventually wanted. Recorded in `src/Hukbo.Client/Content/Audio/PENDING-SOUNDS.md`, which also carries the undesigned swing-sound question. **Phase 3b and Phase 3c fall away with it**; the clash resolution, the swing animation, and the event-log line are unaffected. |
...
| [ ] | T63 | Replace the recorded oracle with the new figures for both workloads, retire the Phase 2 reference-pair heading T34 added, and move **both** superseded pairs into the dead-value table with a one-line reason each. | modify `docs/development/testing.md` | T61, T62 | DOCS | The superseded table names the 200-agent pair `5BEBA7A68F69BE0D` and `D379B60B2E30FFFC` **and** the 500-agent pair `FE44ADA93E0E202A` and `9C8EF5CB79810560`, and the live tables carry only figures produced by T61 and T62. |
...
- At B0 the **entire pre-existing suite is green** and the seed-1 workload still reports
  `5BEBA7A68F69BE0D` and `D379B60B2E30FFFC`, proving the T08 seam changed no value.
...
current 1154 ticks. That runs inside stage 4 of the canonical gate. The expectation is
