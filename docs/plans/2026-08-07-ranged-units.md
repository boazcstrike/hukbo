...
### RU-46 needed two passes, and the second was proven by mutation rather than by a green suite

RU-46's first pass turned the suite green and did not do the job. It correctly replaced
`Assert.Single` with `Assert.Equal(2, ...)`, correctly raised the pass-through count to a
literal 3, and correctly renamed a fact whose name had begun to state something false. But
`PrecolonialPhilippinesV5` and `RangedStandoffV8` appeared in that file only inside a doc
comment. Nothing asserted the second invocation carried them, and nothing asserted the
`if ($Game -eq 'Hukbo')` guard existed at all.

So the pin could not fail for the reason it exists. Deleting both preset arguments, or
deleting the guard, would have left two matching invocations, a count of 2, and a canonical
block still carrying `Agents = 200`, `Ticks = 10000`, `Seed = 1` — green, with the gate
back to running the V4 workload twice and exercising no ranged path. **This is the same
defect the package has now produced seven times in different clothing**, and it is the
first time it arrived as a test rather than as code: something present, documented, and
load-bearing in appearance, with nothing actually reading it.

The second pass added the missing assertions: the ranged block contains both preset
arguments, the guard exists, the guard begins *after* the canonical invocation so a caller
passing no `-Game` still runs it unconditionally, the guard begins *before* the ranged
invocation, and the guard's own brace block contains a `benchmark.ps1` call.

**A green suite is not evidence that a pin works, so it was not accepted as evidence.**
The orchestrator deleted the line `Preset = 'PrecolonialPhilippinesV5'` from
`scripts/verify.ps1` and re-ran the three facts: one failed. The script was then restored
from git and the full Client suite re-run at 0 failed of 3,328. That mutation is the
evidence the pin is live.

One note on doing that check, because the first attempt at it was wrong and briefly looked
like proof of the opposite. The initial mutation used a `sed` pattern with twelve spaces of
indentation against a line that carries eight, so nothing was edited and the suite passed —
which reads exactly like a dead test. The pattern was confirmed against the file, the
mutation redone, and the failure observed. **A mutation test that does not verify the
...
### RU-43 landed, and the shipped game does not play the battle that was calibrated

RU-43 merged. Both suites stay fully green — Core 0 of 2,433, Client 0 of 3,333, format
passes. The sliders work again: `BuildScenario` fills `RosterCounts` through a new
`ExpandCompositionToRosterCounts`, which reads each roster row's `RankId`, groups the rows
by rank, and apportions that rank's slider count across its rows by largest remainder using
the RU-24 and RU-45 weights. Everything derives from `rules.Roster`, so V4 still produces
`[63, 63, 62, 62]`, exactly what it produces today. The anti-dead-feature test is real:
flattening Timawa's five-row split to an even one removes both 8 and 16 from the result and
turns two assertions red.

**But the default composition does not reproduce the calibrated army, and the task did not
notice.** RU-43 reported "proportions MATCH", which is true only *within* a rank. The
overall share is a different question, and it was in the acceptance criteria.

`ArmyComposition.Default` splits evenly across the four ranks — 63, 63, 62, 62 of 250. Under
...
game actually produces, by setting `HUKBO_RANGED_ROSTER_WEIGHTS` to RU-43's expanded array:

| Band | At the calibrated 25% | At the shipped 14% |
| --- | --- | --- |
| (a) share inside 0.25–0.45 | PASS, 20 of 20 | **PASS, 20 of 20** |
| (b) shielded absorb more than shieldless | PASS, 20 of 20 | **PASS on the plan's criterion, 20 of 20** |
| (c) and (d) termination and both factions win | PASS, median 1,415 | **PASS, 20 of 20 decisive, median 1,446, 13 and 7** |

**Every criterion the plan states holds at the shipped composition.** No action is required
on the numbers, and nothing needs retuning. What is required is that nobody later assumes
the shipped default *is* the calibrated army — it is not, and the gap is roughly eleven
percentage points of ranged share.

**The harness said FAIL, and it was wrong to.** Its band (b) check asserts
`shieldedMean > shieldlessMean * 1.15`, a margin RU-45 invented that no plan row requires,
and which this document already flagged as such when RU-45 landed. At the shipped
composition two seeds fall under it — seed 5 at 1.10 and seed 16 at exactly 1.15, which
fails a strictly-greater test — so the harness printed `band b verdict: FAIL` for a run
where all twenty ratios exceed 1.0 and the plan's criterion passes outright.

That is the most dangerous shape of wrong evidence this package has produced: not a green
that hides a defect, but a **red that invents one**. The next person to run this harness
would have concluded the shield had stopped working and gone looking for a regression that
