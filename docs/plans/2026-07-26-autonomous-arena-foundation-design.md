# Autonomous Arena Foundation Design
...
```text
AutonomousArena.Client
        |
        v
AutonomousArena.Core <--- AutonomousArena.Headless
        ^
        |
AutonomousArena.Core.Tests
```

`AutonomousArena.Core` owns all authoritative gameplay state and has no
MonoGame, filesystem, network, GPU, audio, wall-clock, or window dependency.

`AutonomousArena.Client` owns the MonoGame loop, batched dots, camera,
...
`AutonomousArena.Headless` runs deterministic smoke workloads and emits
machine-readable results without opening a window.

`AutonomousArena.Core.Tests` proves deterministic behavior, stable ordering,
