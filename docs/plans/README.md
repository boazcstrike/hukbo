...
| [`2026-07-29-contingent-shape-design.md`](2026-07-29-contingent-shape-design.md) | Contingent shape, Phase C. Corrected on 2026-08-13 against its own evidence base and against the code: four citations misstated the research, two claims were refuted outright, and the design does not mention the determinism hazard that a changed lattice can change whether a jitter draw happens at all | Design only. Its planning pass is done and lives in `2026-08-13-contingent-shape.md`. All four of its open questions are now closed, the fourth on 2026-08-14 by withdrawing section 4's chief-per-contingent claim rather than by answering it |
| [`2026-08-13-contingent-shape.md`](2026-08-13-contingent-shape.md) | The planning pass Phase C asked for: nine ordered tasks, the blast radius across thirteen `FormationPlannerTests` and nine frozen digests, and the finding that `BattlefieldRealismV10` and `LastStandEngagementV11` — the presets the client actually ships — have no frozen trajectory digest at all | **Finished 2026-08-14.** Tasks 1 through 6 shipped; task 7 is closed as accepted rather than delivered, per its section 6b.1 |
| [`2026-08-14-contingent-chief-membership-design.md`](2026-08-14-contingent-chief-membership-design.md) | Prices the three ways out of the contingent-shape package's one undelivered task, with the line that destroys the planner's chief spread quoted, and finds a larger problem underneath them: `ContingentShapeV12` was registered but absent from the client's preset selector, so nothing in the shipped game could reach it | Design only; authorizes nothing. Its recommendation was taken on 2026-08-14 |
| [`2026-08-14-contingent-chief-membership.md`](2026-08-14-contingent-chief-membership.md) | Seven tasks executing that decision: V12 added to the player-facing selector, the Client test that should have caught its absence strengthened to enumerate the registry, and the chief-per-contingent claim withdrawn from the two documents that make it | **Finished 2026-08-14.** Two smoke rows, `CS-1` and `CS-2`, are left `PENDING` for a person |
