...
| [`2026-08-09-ranged-units-handoff.md`](2026-08-09-ranged-units-handoff.md) | The ranged package's closure record. The sound-take listening acceptance closed on 2026-08-14, `RG-11`'s open question was answered on the same day — a projectile passing through a friendly warrior does not look wrong at battle pace — and the default composition moved onto the calibrated proportions. Its eleven `RG-*` smoke rows had already closed `PASS` and left the checklist. Two things survive: the V9 termination gap, accepted with the gap recorded rather than fixed, and Phase 2's line of sight and friendly fire, deferred by the design itself | Closure record; two items survive |
...
| [`2026-08-12-sandata-order-and-combat-legibility.md`](2026-08-12-sandata-order-and-combat-legibility.md) | That design's thirteen tasks across five waves, and the verification each one owed | Build finished and merged; both gates green. All three of its smoke rows closed on 2026-08-14, but only one of them against this package: `SD-7b` passed as built, while `SD-4` and `SD-5` failed a third time here and were closed by the 2026-08-14 package instead. Read that one for what was actually wrong |
