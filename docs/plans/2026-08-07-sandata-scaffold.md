...
### Task 86 complete, and the survivor counts it moved as expected — 2026-08-08

Merged. `CollisionBodyRadiusRaw` moved from the invented 32 raw (0.03125 wu) to
the designed 4,352 raw (4.25 wu), cited to
`Hukbo.Core/Simulation/CollisionRules.cs:72`'s `DefaultBodyRadiusRaw`.
`CollisionCellSizeRaw` moved from 256 raw to 8,704 raw — exactly twice the
body radius, the one-body-diameter rule `SandataCollisionGrid.ValidateBodyRadius`
already enforced at runtime.

**The seed-1 workload re-run, both hashes and both survivor counts moved.**
Before this task (task 79d-1's recorded baseline): event hash
`270364E265A3A8A7`, state hash `6D4AEA08BEFEFA92`, 98 and 92 survivors. After:
event hash `7C1B37876769DEC7`, state hash `BDD56EBD06F76674`, **70 and 64
survivors**, `deterministic: true`. This is the movement task 86's own row
expected and task 79d-1 could not get — a shooter's target now subtends an
angle large enough for range and dispersion to decide outcomes, rather than
whether a drawn error rounds to near zero against a three-centimetre body.
Re-run and verified directly:

```
./scripts/benchmark.ps1 -Game Sandata -Seed 1
```

produced the JSON report above; nothing here is taken from a sub-agent's
report, per the task-79d-1 process note that every count in this document is
re-derived.

**The old 4 wu one-operator-per-cell headless placement put adjacent
operators inside the new 8.5 wu body diameter.** Minimum pairwise separation
at 200 operators, seed 1 was 2.0 wu before the re-space and 10.0 wu after —
`HeadlessRunner.BuildOpenGrid`/`BuildInitialState` now derive the pitch from
the body diameter plus the fixture's own worst-case jitter
(`OperatorSpacingPitchCells = 3`) rather than the fixed 4 wu pitch, and grow
the grid itself to keep every pitched coordinate in bounds. The SplitMix64
draw sequence is unchanged — same calls, same order, same count — only the
placement arithmetic moved.

**A miss became mathematically unreachable through `RunTick` for the AK-47
loadout.** At the designed radius the dispersion/half-angle crossover sits at
roughly 345 wu, past `ContactMemory.DetectRangeWu` (256), so no sensing-reachable
geometry can ever draw one. Task 79d-1's `RunTick_Miss_...` test is replaced
with an id-invariant hit check
(`RunTick_SameGeometryAnyShooterEntityId_AlwaysHits`, ids 2 and 25) and a
regression proof that pins the impossibility
(`SubtendedHalfAngle_AlwaysAtLeast_AkDispersion_WithinDetectRange`) so a future
change that reopens a reachable miss fails a test rather than going unnoticed.
This is a genuine contradiction of task 79d-1's original assumption that
shooter-id tuning alone keeps both a hit and a miss path reachable, reported
per the task-86 brief's evidence-contradicts-brief rule rather than papered
over.

Re-derived test totals, Release configuration: `Sandata.Core.Tests` 1,088
passed, 0 failed; `Sandata.Client.Tests` 199 passed, 0 failed (unchanged).
`Sandata.Core.Tests` grew by exactly three facts over task 79d-1's recorded
1,085: `CollisionBodyRadiusAndCellSize_MatchTheDesignedValues` and
`HeadlessFixture_MinimumPairwiseSeparation_ClearsOneBodyDiameter`, both new,
plus `SandataCollisionTests`' new body-radius-changes-collision fact. The old
`RunTick_Miss_EmitsExactlyOneShotFiredAndOneShotMissedEvent` `[Fact]` was
repurposed in place into
`SubtendedHalfAngle_AlwaysAtLeast_AkDispersion_WithinDetectRange` rather than
deleted and re-added, so it does not change the count.

`./scripts/verify.ps1` is the canonical gate and does not cover Sandata (recorded
at wave 10); this task's evidence is the direct `dotnet test` and
`benchmark.ps1` runs above, not that gate.
