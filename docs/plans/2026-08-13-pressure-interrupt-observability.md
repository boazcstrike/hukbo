...
| PO-2 | Persist it: schema bump to 9, default `BattlefieldRealismV10`, per-field fallback that also rejects an unregistered id | `src/Hukbo.Client/Settings/ClientSettingsStore.cs` | A schema-8 file still loads; an unregistered id falls back rather than reaching `Scenario.Validate` | PO-1 |
...
| PO-4 | `BuildScenario` reads the staged preset instead of the hardcoded `BattlefieldRealismV10` | `src/Hukbo.Client/ArenaGame.cs:1435-1452`, apply path at `:1350-1372` | With untouched settings the scenario still names `BattlefieldRealismV10`; the change applies only on Full Reset | PO-2, PO-3 |
